...
export const CreateTaskModal: React.FC<CreateTaskModalProps> = ({ isOpen, onClose, defaultProjectId, defaultStatus }) => {
  ...
  const [projectId, setProjectId] = useState(defaultProjectId || mockProjects[0].id);
  const [assigneeId, setAssigneeId] = useState(user?.id || mockUsers[0].id);
  ...
  const resetForm = () => {
    setTitle('');
    setDescription('');
    setProjectId(defaultProjectId || mockProjects[0].id);
    setAssigneeId(user?.id || mockUsers[0].id);
    setPriority('medium');
