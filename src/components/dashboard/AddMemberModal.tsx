...
export const AddMemberModal: React.FC<AddMemberModalProps> = ({ isOpen, onClose, project }) => {
  const { updateProject, users } = useAppContext();
  ...
  const handleSave = async () => {
    ...
    try {
      await updateProject(project.id, { memberIds: selectedIds });
      onClose();
    } finally {
    ...
    >
      <div className="space-y-6">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <img src={u.avatarUrl ?? undefined} className="w-8 h-8 rounded-full" referrerPolicy="no-referrer" />
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-border">
          ...
          <Button onClick={handleSave} isLoading={isSubmitting} className="gap-2">
